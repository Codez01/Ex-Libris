...
document.getElementsByName("gsft_main")[0].contentWindow.run = function(fn){ fn.call(this, this.window); };
...
var PageCapacity = parseInt(window.document.querySelectorAll(".vt").length) ; //Page Incidents Total number
...
for(var i = 1  ; i < window.document.querySelectorAll("th").length; i++){

  if(((window.document.querySelectorAll("th")[i].innerText).includes("Title")) ){
     tempTitle = counter2;
    // console.log("title : " + tempTitle);
   
  }
  if(((window.document.querySelectorAll("th")[i].innerText).includes("Configuration item")) ){
     tempConfig = counter2;
    //console.log("\nConfig : "+tempConfig);
   
  }  
  if(((window.document.querySelectorAll("th")[i].innerText).includes("Close code")) ){
     tempClose = counter2;
    //console.log("\nClose code : " + tempClose);
    
  }  
  if(((window.document.querySelectorAll("th")[i].innerText).includes("On-Call Assigned to")) ){
     tempAssignedTo= counter2;
    
    
  }  
  if(((window.document.querySelectorAll("th")[i].innerText).includes("On-Call Assignment group")) ){
     tempAssignmentgroup = counter2;
  
  }  
  if(((window.document.querySelectorAll("th")[i].innerText).includes("Contact type")) ){
    tempContactType = counter2;
   
 
 }  
 if(((window.document.querySelectorAll("th")[i].innerText).includes("Number")) ){
  tempNumber = counter2;


}
 
  if(((window.document.querySelectorAll("th")[i].innerText).includes("Update Personalized List") || (window.document.querySelectorAll("th")[i].innerText).includes("Personalize List") ) ){
    column = counter2;
    //console.log("\ncolumn : " + column);
    break;
  } 
  else{
    //console.log(window.document.querySelectorAll("th")[i].innerText);
counter2++;
  }
}
...
//-------------------------------------------------------------------
    ...
    string = window.document.querySelectorAll(".vt")[index].innerText;//string that contains the specific incident title.
    CloseCode = window.document.querySelectorAll(".vt")[CloseIndex].innerText; // it gets the closeCode information about each incident.
    var configurationItem = window.document.querySelectorAll(".vt")[ConfigurationIndex ].innerText;

    if((window.document.querySelectorAll(".vt")[OnAssignmentGroup].innerText).includes("(empty)")){
      OnAssignmentGroup="";
      AssignedTo="";
    }else{
     
      OnAssignmentGroup = "   ( Assigned Group : " + window.document.querySelectorAll(".vt")[OnAssignmentGroup].innerText;
     AssignedTo = "  / Assigned To : " + window.document.querySelectorAll(".vt")[AssignedTo].innerText+" )";
    }
   ...
   if(configurationItem.includes("(empty)")){
    ...
    if (string.includes(substring2) || string.includes("iii")) {// iii incidents
      if(CloseCode.includes(substring4)){
        FalseAlerts.push("\n           • " + window.document.querySelectorAll(".vt")[index].innerText + "        " + configurationItem + "        " +CloseCode+ OnAssignmentGroup + AssignedTo);
      }
      III.push("\n               • " + window.document.querySelectorAll(".vt")[index].innerText + "        " + configurationItem + "        " +CloseCode+ OnAssignmentGroup+ AssignedTo);
    }
    if ((window.document.querySelectorAll(".vt")[contact_type].innerText).includes(substring3)) {//salesforce incidents
      if(CloseCode.includes(substring4)){
        FalseAlerts.push("\n           • " + window.document.querySelectorAll(".vt")[index].innerText + "        " + configurationItem + "        " +CloseCode+ OnAssignmentGroup)+ AssignedTo;
      }
     
      SalesForce.push("\n          • " + window.document.querySelectorAll(".vt")[number].innerText + "        " + window.document.querySelectorAll(".vt")[index].innerText + "        " + configurationItem + "        " +CloseCode+ OnAssignmentGroup+ AssignedTo);
      ...
      if (string.includes(substring)) {//if a Pingdom was found in within the incident title
        ...
        Pingdom.push("\n           • " + window.document.querySelectorAll(".vt")[index].innerText + "        " + configurationItem + "        " +CloseCode+ OnAssignmentGroup+ AssignedTo);//add the pingdom title and other elements to the array
...
console.log("%c" + "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n **  The Number Of The Copied Alerts Are: " + counter + "/" + (parseInt(window.document.querySelectorAll(".vt").length) / (column)) + "  **", "color:" + "DodgerBlue");
